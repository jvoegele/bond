...
Bond has shipped agent-facing rules since 1.17.0, written for the agent that reads them.
This release adds the missing half: a guide for the person who has to install them.
...
- **`@invariant` works on a module whose clause heads are generated by a macro — `Ecto.Schema`
  above all.** Adding a single `@invariant` to an Ecto schema failed to compile:

  ```
  error: undefined variable "bond_arg_1"
    └─ lib/account.ex:1: Account.__schema__/2
  ```

  Nothing about it was Ecto-specific. Invariants weave into every public function of the
  declaring module, so they reach heads built with `unquote_splicing/1` — which is how
  `Ecto.Schema` emits `__schema__/1,2`. Variables in such a head carry the `quote`-introduced
  hygiene context rather than the module's, and a variable's identity is name *and* context. The
  wrapper bound its canonical name in the head's context while referencing it from the body in
  `nil`, so the head never bound the variable the body used. The wrapper now always binds in
  `nil`, wrapping as `canonical = <original pattern>` rather than rewriting the head variable in
  place — the user's own binding has to survive for a reproduced `when` guard to still resolve.

  This ruled out `@invariant` on the most common struct type in a Phoenix application, which is
  also where the [invariants guide](guides/invariants.md) sends you.

- **Functions whose names are wrapped in double underscores are no longer woven.** Bond already
  skipped `__struct__`, `__exception__`, and `__info__` as compiler-generated; it now skips
  anything following that naming convention, which is what libraries use for generated and
  introspection functions. Weaving into them was never meaningful — none takes or returns the
  declaring struct — and it cost a wrapper clause per generated clause on functions like
  `__schema__/2` that Ecto calls while building every query, plus a "never mentions the struct"
  warning apiece, pointing at the `schema` macro rather than at anything the author wrote.

  Two consequences worth calling out:

  * **The one-`@contract`-per-module limit on Norm coexistence is gone.** Norm emits a
    `def __contract__/1` helper per `@contract`, and two of them produced non-adjacent clauses
...
- **`Bond.Server` state and transition invariants now appear in `Bond.Coverage`.**
  ...
  invariant fails on every message. The coverage row is the signal a failing test would normally
  be, and both were missing at once. `testing-contracts.md` now says so.

  Module-level server invariants carry no `:function` (one declaration is checked around every
  callback), so the report headed their group with a bare `nil` once they started being recorded.
  It now reads `(every callback)`.

- **Coverage rows are attributed to the module and function that actually ran them.** Entries
  were keyed on `assertion_id` alone. An inherited contract is a single assertion created once in
  the declaring behaviour or protocol, so every implementing module shares its id; an
  `@invariant` is one assertion woven into every public function of its module. All of those
  collapsed into one row — and since the metadata was written only as the `:ets.update_counter`
  default, that row was attributed to whichever module or function recorded first, which under
  ExUnit depends on the seed.

  The consequence was worse than a missing breakdown: a `✓` could be earned by a different module
  than the one it was printed against, which reads as "this correct implementation violated its
  contract". Rows are now keyed on the assertion together with the module, function, kind, and
  label — every field a printed row is identified by — so six implementations of a behaviour give
  six rows, each true of the module it names.

- **The all-inside `where`/`whenever` form now works on inherited contracts.**
  `guides/public-api.md` documents `where(binding, assertions…)` as an alias of the prefix form
  `where(binding), assertions…`, and says so specifically for `Bond.Behaviour` callbacks and
  `Bond.Protocol` functions. Both rejected it:

  ```elixir
  @post whenever(pat <- result), ok: pat >= 0   # compiled
  @post whenever(pat <- result, ok: pat >= 0)   # ** (CompileError)
  ```

  The `@` clause on each inherited path matched only a binder call carrying exactly one argument,
  so the all-inside shape fell through to the bare-expression clause and was rejected by the
  check for a *nested* binder. The diagnostic was therefore the one written for a genuinely
  nested form — "`whenever` may only appear at the start of a contract" — which read as "you
  wrote this in the wrong place" when the binder was at the start and none of its three suggested
  rewrites applied.

  `Bond.Protocol` was affected as well as `Bond.Behaviour`, and `where` as well as `whenever`.
  `@pre_weaken` / `@post_strengthen` on a protocol implementation still do not accept either
  binding form; that remains deliberate (#47) and undocumented as supported.

- **A long assertion no longer breaks out of its code block in the generated contract docs.**
  `Bond.Compiler.ContractDocs` emitted the `#### Preconditions` / `#### Postconditions` and
  `## Invariants` sections as 4-space-indented Markdown code blocks, indenting each *assertion*
  on the assumption that one assertion is one line. But the rendered form is `Macro.to_string/1`
  output, so it runs the formatter and **wraps** once it exceeds the line length — and the
  continuation arrives carrying the formatter's own indentation, 0 or 2 spaces, below Markdown's
  4-space threshold. The block ended there.

  One cause, two symptoms: at 2 spaces the continuation rendered as an indented paragraph in the
  body font with a fresh code block opening after it; at 0 it was folded into the preceding line
  with no separator, which looked like a whitespace bug around `~>` and was not one — the newline
  had been eaten by paragraph folding.

  Both sections are now emitted as fenced ` ```elixir ` blocks, which are immune to indentation
  and pin the lexer regardless of the expression's content. This is a **visible change to
  generated documentation**: the contract sections are now fenced rather than indented, matching
  how `@spec` is already rendered directly above them on the same page.
