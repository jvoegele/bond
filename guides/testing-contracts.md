...
## Contract density — which functions carry a contract at all?

Coverage reports assertions that **ran**, which makes it a report about contracts that
*exist*. A function nobody contracted appears nowhere in it, and neither does the module it
lives in. That absence is invisible by construction, and it is the failure worth worrying
about: a codebase with too few contracts looks exactly like one that never needed them.

`mix bond.audit` is the other half. It reads a reflection the compiler emits into every module
that `use Bond`, so it knows exactly what was contracted — no source parsing, which means it
sees functions generated by macros, modules that acquired `use Bond` through some other
`__using__`, and `defdelegate`s.

```
$ mix bond.audit

Bond contract density — 3 modules using Bond
  ...
  4 of 7 carry no contract of their own.
  1 covered by a module invariant only; 0 callback(s) counted separately.
...
| Switch | Effect |
| --- | --- |
| `--verbose` | List every uncontracted function, not just the per-module tally |
...
are separated, because what a callback promises is the *behaviour's* to state (see
[Contract Inheritance](contract-inheritance.md)); and a function covered only by a module
`@invariant` is separated, because an invariant is a law about the type rather than a promise
about any one function, and merging the two flatters the number.

Like the coverage table, it is a prompt and not a verdict. A bare function is the question
"what does this promise?", and some honest answers are "nothing beyond its `@spec`". The
reasons to decline are short — the assertion would be unsound, unreachable, or not warranted by
the specification — and "there are a lot of them" is not among them, so a large count is a
backlog rather than a defect. See [What Should a Contract Say?](what-contracts-say.md).
...
## See also
  ...
  * [What Should a Contract Say?](what-contracts-say.md) — what to do with the functions
    `mix bond.audit` reports bare.
