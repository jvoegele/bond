defmodule Bond.Compiler do
  ...
  defmacro __before_compile__(%Macro.Env{} = env) do
    ...
    contracted_ast = build_contracted_reflection(annotated)
  ...
  # Emits the `__bond_contracted__/0` reflection: every function this module contracted, with a
  # count of each kind of assertion attached to it.
  #
  # `Bond.Coverage` reports assertions that *ran*, which makes it a report about contracts that
  # exist — a function nobody contracted appears nowhere in it, and neither does the module it
  # lives in. That is the one thing coverage structurally cannot show, and it is the failure the
  # usage rules call one-sided: a codebase with too few contracts looks exactly like one that
  # never needed them.
  #
  # The compiler is the only place that knows the answer exactly. Deriving it from source instead
  # means parsing, which cannot see a function generated by a macro, a module that acquired
  # `use Bond` through some other `__using__`, or a `defdelegate`. All of that is already in
  # `annotated`; this stops throwing it away.
  #
  # `own_assertions` distinguishes a function with its own `@pre`/`@post` from one that appears
  # here only because a module `@invariant` is checked around it. Both are coverage, and they are
  # not the same claim: an invariant is a law about the *type*, not a promise about any one
  # function, and `mix bond.audit` reports them apart for that reason.
  #
  # Emitted even when the module contracted **nothing**, and that is the point rather than an
  # oversight: `__bond_contracted__() == %{}` is how an audit tells "uses Bond and contracts
  # nothing" — the most interesting row it can print — from "does not use Bond", which is a
  # different question with a different answer. Exporting it unconditionally also makes
  # `function_exported?/3` a reliable test for the first.
  defp build_contracted_reflection(annotated) do
    table =
      Map.new(annotated, fn function ->
        pre = length(function.preconditions) + length(function.pre_weaken_assertions)
        post = length(function.postconditions) + length(function.post_strengthen_assertions)
        ...
        {{function.fun, function.arity},
         %{
           ...
           preconditions: pre,
           postconditions: post,
           invariants: length(function.invariants),
           own_assertions: pre + post
         }}
      end)

    quote do
      @doc false
      def __bond_contracted__, do: unquote(Macro.escape(table))
    end
  end
